...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchProjects() {
      // Simulando busca do Supabase para manter o profissionalismo
      // No futuro: const { data } = await supabase.from('projects').select('*');
      const mockProjects = [
    ...
    <div className="min-h-screen bg-[#020202] text-foreground flex flex-col font-body pb-20 md:pb-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
               <span className="text-[9px] text-white/30 font-black tracking-widest uppercase">Mobile_Studio v2.5</span>
               <div className="flex items-center gap-1 bg-green-500/10 px-1.5 py-0.5 rounded border border-green-500/20">
                 ...
                 <span className="text-[8px] font-bold text-green-500 uppercase">Supabase_Link</span>
               </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 p-6 md:p-12 max-w-7xl mx-auto w-full space-y-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-white/40 font-medium text-sm md:text-base italic">Local engineering with Cloud Persistence.</p>
          </div>
        ...
        <Tabs defaultValue="all" className="w-full">
          <div className="flex items-center justify-between mb-8 overflow-x-auto custom-scrollbar">
            <TabsList className="bg-white/5 border border-white/10 p-1 rounded-2xl h-14">
              <TabsTrigger value="all" className="data-[state=active]:bg-primary data-[state=active]:text-black rounded-xl px-8 font-bold text-xs uppercase tracking-widest">Local_Cache</TabsTrigger>
              <TabsTrigger value="cloud" className="data-[state=active]:bg-primary data-[state=active]:text-black rounded-xl px-8 font-bold text-xs uppercase tracking-widest">Cloud_Vault</TabsTrigger>
            </TabsList>
          </div>
          ...
          <TabsContent value="all" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 m-0">
            {isLoading ? (
              Array.from({length: 3}).map((_, i) => (
                <div key={i} className="h-[400px] rounded-3xl bg-white/5 animate-pulse border border-white/5" />
              ))
            ) : (
              projects.map((p) => (
                  ...
                  <div className="relative aspect-video overflow-hidden">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-300">
                      <Link href={`/editor/${p.id}`}>
                        <Button className="rounded-2xl font-bold bg-white text-black h-14 px-8 shadow-2xl uppercase tracking-widest text-xs">
                          ENTER_STUDIO
                        </Button>
                      </Link>
                    </div>
                  ...
                  <CardHeader className="p-6">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-white/30 uppercase tracking-widest">{p.type}</span>
            ...
            <Link href="/editor/new">
              <div className="border-2 border-dashed border-white/10 rounded-3xl h-full min-h-[350px] flex flex-col items-center justify-center p-8 text-center space-y-4 hover:bg-white/5 hover:border-primary/30 transition-all group relative">
                <div className="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity" />
                <div className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center group-hover:scale-110 transition-all shadow-xl">
                  <Plus className="h-8 w-8 text-white/20 group-hover:text-primary" />
                </div>
                <div className="space-y-1 z-10">
                  <h3 className="font-headline font-bold text-lg text-white uppercase italic">NEW_GENESIS</h3>
                  <p className="text-xs text-white/30 font-medium max-w-[180px] mx-auto uppercase tracking-tighter">Forge a new reality via Neural Engine.</p>
                </div>
              </div>
            </Link>
          </TabsContent>
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-20 bg-black/80 backdrop-blur-2xl border-t border-white/5 flex items-center justify-around px-6 md:hidden z-[100]">
        <Link href="/dashboard" className="text-primary flex flex-col items-center gap-1">
          ...
          <span className="text-[9px] font-bold uppercase tracking-widest">Cores</span>
        </Link>
        <Link href="/editor/new" className="bg-primary p-4 rounded-2xl -translate-y-8 shadow-2xl glow-primary text-black transition-transform active:scale-90">
