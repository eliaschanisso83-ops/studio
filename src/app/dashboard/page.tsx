          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
               <div className="flex items-center gap-1 bg-green-500/10 px-1 py-0.5 rounded border border-green-500/20">
                 ...
                 <span className="text-[6px] font-bold text-green-500 uppercase tracking-widest">Supabase_OK</span>
               </div>
            </div>
          </div>
        </div>
      ...
      <main className="flex-1 p-4 md:p-6 max-w-5xl mx-auto w-full space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">Armazenamento_Local</h2>
          </div>
          <Link href="/editor/new">
            <Button size="sm" className="bg-primary hover:bg-primary/90 text-black font-headline font-bold h-8 px-4 rounded-lg shadow-lg text-[9px] uppercase tracking-widest">
              <Plus className="mr-1 h-3 w-3" /> Genesis
            </Button>
          </Link>
        </div>

        <Tabs defaultValue="all" className="w-full">
          <TabsList className="bg-white/5 border border-white/10 p-0.5 rounded-lg h-8 mb-4">
            <TabsTrigger value="all" className="data-[state=active]:bg-primary data-[state=active]:text-black rounded px-3 font-bold text-[8px] uppercase tracking-widest h-full">Cores_Library</TabsTrigger>
            <TabsTrigger value="cloud" className="data-[state=active]:bg-primary data-[state=active]:text-black rounded px-3 font-bold text-[8px] uppercase tracking-widest h-full">Vault_Link</TabsTrigger>
          </TabsList>
          
          <TabsContent value="all" className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {isLoading ? (
              Array.from({length: 4}).map((_, i) => (
                <div key={i} className="aspect-[4/3] rounded-xl bg-white/5 animate-pulse border border-white/5" />
              ))
            ) : (
              projects.map((p) => (
                <Card key={p.id} className="glass-panel group overflow-hidden border-white/5 hover:border-primary/40 transition-all duration-300 rounded-xl flex flex-col">
                  <div className="relative aspect-[16/10] overflow-hidden">
                    ...
                    <div className="absolute top-1 right-1">
                      <span className="text-[6px] px-1 py-0.5 rounded bg-black/80 border border-white/10 text-primary font-black uppercase tracking-widest">
                        {p.status}
                      </span>
                    </div>
                  </div>
                  ...
                  <CardFooter className="p-2 pt-1 flex justify-between items-center mt-auto border-t border-white/5 bg-white/[0.01]">
                    <div className="flex items-center text-white/20 text-[6px] font-bold uppercase">
                    ...
                    <div className="flex gap-1.5">
                      <Button 
                         ...
                         variant="ghost" 
                         size="icon" 
                         className="h-5 w-5 rounded-md hover:bg-destructive/20 hover:text-destructive text-white/20"
                       >
                         <Trash2 className="h-3 w-3" />
                       </Button>
                       <Link href={`/editor/${p.id}`}>
                         <Button size="icon" className="h-5 w-5 rounded-md bg-primary/10 text-primary hover:bg-primary hover:text-black">
            ...
            <Link href="/editor/new">
              <div className="border border-dashed border-white/10 rounded-xl aspect-[16/10] flex flex-col items-center justify-center p-3 text-center space-y-1 hover:bg-white/5 hover:border-primary/30 transition-all group">
                <Plus className="h-4 w-4 text-white/20 group-hover:text-primary group-hover:scale-110 transition-all" />
                <h3 className="font-headline font-bold text-[8px] text-white/40 uppercase tracking-widest">NOVO_PROJETO</h3>
              </div>
            </Link>
          </TabsContent>
        </Tabs>
